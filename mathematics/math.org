...
2. 重复 E-step 和 M-step 直到收敛
   ...
   2. M-step: 最大化似然函数从而获得新的参数值:
      ...
      #+BEGIN_SRC latex :results raw :exports none
        \begin{equation}
          \theta:=arg\max\limit_{\theta}\sum_{i}\sum_{z^{(i)}}Q_{i}(z^{(i)})\log\frac{p(x^{(i)},z^{(i)};\theta)}{Q_{i}(z^{(i)})}
        \end{equation}
      #+END_SRC 
...
[[file:image/Machine%20learning/screenshot_2018-10-22_20-31-15.png]]
...
上面推导的最后一步使用了 [[Jensen]] 不等式。其中的 $f(x)=\log(x)$ 是一个凹函数 $E(f(X))\leq f(E[X])$ , 因为其二阶导数 $f''(x)=\frac{-1}{x^{2}}\leq 0$ 在整个定义域 $x\in R^{+}$ 上都成立
上式的求和中的单项: 
<<Eqn.(1.5.1.4)>>
#+BEGIN_SRC latex :results raw :exports none
  \begin{equation}
...
是变量 $\frac{p(x^{(i)},z^{(i)};\theta)}{Q_{i}(z^{(i)})}$ 基于 $z^{(i)}$ 的期望, 其中 $z^{(i)}$ 是根据 $Q_{i}$ 给定的分布确定。然后利用 [[Jensen]] 不等式就得到了
...
为了使上式相等, 只需确保对一个常数值随机变量求期望([[Jensen]]):
...
<<Eqn.(1.5.1.7)>>
n#+BEGIN_SRC latex :results raw :exports none
  \begin{equation}
...
[[file:image/Deep%20learning/screenshot_2018-12-27_19-45-23.png]]
...
[[file:image/Deep%20learning/screenshot_2018-12-27_20-44-44.png]]
...
[[file:image/Deep%20learning/screenshot_2019-01-01_19-27-15.png]]
[[file:image/Deep%20learning/screenshot_2019-01-01_19-27-29.png]]
