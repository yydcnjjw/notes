...
7. 跳转到 =x86_64_start_kernel= (arch/x86/kernel/head64.c, c code)
...
7. =setup_arch=: architecture-dependent parts of initialization
   ...
   29. =idt_setup_early_pf()=: Initialize the idt table with early pagefault handler.(head64.c has Initialized ?)
...
8. =mm_init_cpumask()=: set the [[cpumask]] pointer to the memory descriptor =cpumask= (clear 0).
9. =setup_command_line()=: takes pointer to the kernel command line allocates a couple of buffers to store command line.
...
10. =setup_nr_cpu_ids()=: set =nr_cpu_ids= (number of CPUs) according to the last bit in the =cpu_possible_mask=
11. =setup_per_cpu_areas()=: setups memory areas for the =percpu= variables.
12. =smp_prepare_boot_cpu()=: =arch/x86/kernel/smpboot.c=
...
13. =build_all_zonelists(NULL)=: set up the order of zones that allocations are preferred from
14. =page_alloc_init()=: setup the =startup= and =teardown= callbacks for the =CPUHP_PAGE_ALLOC_DEAD= cpu hotplug state.
15. =parse_early_param()=: handle linux kernel command line for no all architecture.
16. =jump_label_init()=: initialize [[https://lwn.net/Articles/412072/][jump label]]
17. =setup_log_buf()=: setup the =printk= log buffer
18. =vfs_caches_init_early()=: do early initialization of the virtual file system
19. =sort_main_extable()=: sort the kernel's built-in exception table entries which are between =__start___ex_table= and =__stop___ex_table=
20. =trap_init()=: initialize trap handlers
21. =mm_init()=: initialization of the memory manager
...
22. =sched_init()=: Scheduler initialization
...
23. *TODO* =preempt_disable()=: disable preemption
24. check IRQs state and disabling
25. +Initialization of the integer ID management+
26. =radix_tree_init()=: initialize kernel implementation of the [[http://en.wikipedia.org/wiki/Radix_tree][Radix tree]]
27. [[RCU]] initialization:
    ...
    3. =rcu_init_one()=: 
28. =trace_init()=: initialize [[https://en.wikipedia.org/wiki/Tracing_%28software%29][tracing]] subsystem
29. =acpi_early_init()=:
30. =thread_stack_cache_init()=: allocates cache for the =thread_stack_cache=
31. =cred_init()=: allocate cache for the credentials(like =uid=, =gid=, etc)
32. =fork_init()=: 
...
33. =proc_caches_init()=: allocate caches for the memory descriptors(or =mm_struct= structure)
...
35. =vfs_caches_init()=:
36. =pagecache_init()=: initialize the ratio for the dirty pages.
37. =signals_init()=: allocate a cache for the =sigqueue= structures which represents queue of the real time signals.
38. =proc_root_init()=: create the root for the [[https://en.wikipedia.org/wiki/Procfs][procfs]]
39. =cgroup_init()=: make initialization of the rest of the cgroup subsystem
40. =taskstats_init_early()=: export per-task statistic to the user-space
41. =delayacct_init()=: initializes per-task delay accounting
42. =check_bugs()=: fix some architecture dependent bugs
43. =rest_init()=:
