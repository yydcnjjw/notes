...
22. =trap_init()=: initialize trap handlers
...
31. =acpi_early_init()=:
32. =thread_stack_cache_init()=: allocates cache for the =thread_stack_cache=
33. =cred_init()=: allocate cache for the credentials(like =uid=, =gid=, etc)
34. =fork_init()=: 
...
35. =proc_caches_init()=: allocate caches for the memory descriptors(or =mm_struct= structure)
...
37. =vfs_caches_init()=:
38. =pagecache_init()=: initialize the ratio for the dirty pages.
39. =signals_init()=: allocate a cache for the =sigqueue= structures which represents queue of the real time signals.
40. =proc_root_init()=: create the root for the [[https://en.wikipedia.org/wiki/Procfs][procfs]]
41. =cgroup_init()=: make initialization of the rest of the cgroup subsystem
42. =taskstats_init_early()=: export per-task statistic to the user-space
43. =delayacct_init()=: initializes per-task delay accounting
44. =check_bugs()=: fix some architecture dependent bugs
45. =rest_init()=:
