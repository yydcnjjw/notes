...
|           | (most frequently, the user logged out)            |                          |
|-----------+---------------------------------------------------+--------------------------|
...
| SIGSEGV   | Memory access                                     | Terminate with core dump |
...
|-----------+---------------------------------------------------+--------------------------|
| SIGVTALRM | Generated by setitimer() when called with         | Terminate                |
|           | the ITIMER_VIRTUAL flag                           |                          |
|-----------+---------------------------------------------------+--------------------------|
...
handler parameter:
- =SIG_DFL=: the behavior of the signal given by =signum= to its default
- =SIG_IGN=: Ignore the signal given by =signum=
- the address of a programmer-defined function(a "signal handler")

man:pause(2): wait for signal

*** Signal Inherited
#+caption: Inherited signal behavior
...
*** Mappping Signal Numbers to Strings
- retrieve the string from a statically defined list =sys_siglist[]=
- man:psignal(3)
- man:strsignal(3)
** Sending a Signal
man:kill(2): send signal to a process
In order to send a signal to another process, the sending process needs proper permissions. A process with the =CAP_KILL= capability. Without this capability, the sending process's effective or real user ID must be equal to the real or saved user ID of the receiving process.
- man:raise(3): send a signal to the caller
- man:killpg(3): send signal to a process group

** Reentrancy
=reentrant function=: It must not manipulate static data, must manipulate only stack-allocated data or data provided to it by the caller, and must not invoke any nonreentrant function
...
** Blocking Signals
man:sigprocmask(2)
man:sigpending(2): retrieving pending signals
man:sigsuspend(2): waiting for a set of signals

** Advanced Signal Managemet
man:sigaction(2)
man:sigqueue(2)
...
** time info
get time
- man:clock_getres(2): find the resolution(precision) of the specified clock(=clk_id=)
- man:time(2): get time in seconds
- man:gettimeofday(2): 
- man:clock_gettime(2): obtain the time of a specific time source
- man:times(2): get process times

set time
- man:stime(2): set time
- man:settimeofday(2): set time with precision
- man:clock_settime(2):

convert between broken-down time and =time_t=
- man:asctime(3): =tm= -> ascii string
- man:mktime(3): =tm= -> time_t
- man:ctime(3): =time_t= -> ascii representation
- man:gmtime(3): =time_t= -> =tm=
- man:localtime(3): =timep= -> broken-down time representation

man:difftime(3): calculate time difference

** tuning the system clock
man:adjtime(3): correct the time to synchronize the system clock

** Sleeping and Waiting
- man:sleep(3): sleep for a specified number of seconds
- man:usleep(3)
- man:nanosleep(3)
- man:clock_nanosleep(2)
- man:select(2): provide a portable way to sleep with subsecond resolution.
  #+BEGIN_SRC C
    struct timeval tv = {
      .tv_sec = 0,
      .tv_usec = 747
